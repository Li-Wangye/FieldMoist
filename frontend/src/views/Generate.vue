      ...
      <form @submit.prevent="submitJob">
        ...
        <div v-if="mode === 'shp'" class="field">
          ...
          <template v-else>
            ...
            <button v-if="name && !uploadedPolygon && !shpUploading" class="saved-boundary-button" type="button" @click="useSavedBoundary"><i class="fa-solid fa-folder-open" /> Use saved boundary</button>
            <small v-if="uploadedPolygon && !testPreset">Saved boundary: {{ uploadedPolygon.name }}</small>
...
const boundaryReady = computed(() => mode.value === 'shp'
  ? (Boolean(testPreset.value) || (!shpUploading.value && uploadedPolygon.value?.name === name.value))
...
async function uploadShpBoundary() {
  ...
  feedback.value = '';
  const areaName = name.value;
  try {
    const existingPolygon = await findSavedBoundary(areaName);
    if (existingPolygon) {
      if (name.value !== areaName || mode.value !== 'shp') return;
      uploadedPolygon.value = existingPolygon;
      feedbackType.value = 'success';
      feedback.value = `Saved boundary for ${areaName} is ready.`;
      return;
    }
...
async function findSavedBoundary(areaName) {
  try {
    const response = await api.get('/api/polygons/lookup/', { params: { name: areaName } });
...
async function useSavedBoundary() {
  if (mode.value !== 'shp' || !name.value || shpUploading.value) return;
  const areaName = name.value;
  shpUploading.value = true;
  feedback.value = '';
  try {
    const polygon = await findSavedBoundary(areaName);
    if (name.value !== areaName || mode.value !== 'shp') return;
    if (!polygon) throw new Error(`No saved boundary found for ${areaName}. Select the SHP files to create it.`);
    uploadedPolygon.value = polygon;
    feedbackType.value = 'success';
    feedback.value = `Saved boundary for ${areaName} is ready.`;
  } catch (error) {
    feedbackType.value = 'error';
    feedback.value = error.response?.data?.detail || error.message;
  } finally {
    shpUploading.value = false;
  }
}
...
.saved-boundary-button { min-height: 32px; display: inline-flex; align-items: center; gap: 7px; margin-top: 8px; padding: 0 10px; color: #285c3e; background: #e7f3eb; border: 1px solid #b8d7c1; border-radius: 4px; font-size: 11px; font-weight: 650; }
.saved-boundary-button:hover { background: #dceee2; }
