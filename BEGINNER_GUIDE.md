# FieldMoist Beginner Installation and Startup Manual

Language: English | [Chinese](新手安装与使用手册.md). A printable [HTML edition](BEGINNER_GUIDE.html) is included and can be opened by double-clicking it.
...
PowerShell users should use `cd C:\FieldMoist\public` instead of `cd /d ...`, and `Copy-Item` instead of `copy`. Do not mix shell-specific commands without adapting them.
...
### 15.1 Complete first-time Earth Engine authentication

Online processing requires an Earth Engine-enabled Google Cloud project and an account with access to it. Obtain the actual project identifier and enter it after `EARTH_ENGINE_PROJECT=` in the backend `.env`.

In the activated FieldMoist prompt, from public:
...
For service-account deployments, the configuration also supports a service-account email and JSON credential path. Beginners should have the provider configure these; keep credentials outside version control.

### 15.2 Prepare inputs and select the area

Extract the online ancillary data listed in section 7.5. Restart the backend after changing `.env`. In Generate, choose **Online**, enter a region name and valid dates, then define the area:
...
The 82 and 83 data ZIPs include NASA caches and are extracted into `public/backend/media`. Existing valid cache cells and date ranges are reused without downloading. Missing coverage needs internet. Online processing still requires GEE access and ancillary data. Already complete local inputs do not need to be extracted again for a routine code update.
