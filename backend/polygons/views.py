...
class PolygonViewSet(viewsets.ModelViewSet):
    ...
    def lookup(self, request):
        """Find an existing area without transferring its full boundary."""
        name = str(request.query_params.get('name', '')).strip()
        if not name:
            return Response({'detail': 'Provide an area name.'}, status=status.HTTP_400_BAD_REQUEST)
        polygon = Polygon.objects.filter(name=name).values('id', 'name').first()
        if polygon is None:
            return Response({'detail': 'No saved boundary has this area name.'}, status=status.HTTP_404_NOT_FOUND)
